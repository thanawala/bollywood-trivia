...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (searchMode === 'local') {
      ...
      // Filter by word count AND ensure it hasn't been used this session
      ...
      if (filtered.length === 0) {
        const totalInCategory = names.filter(name => name.split(/\s+/).filter(Boolean).length >= minWords).length;
        if (totalInCategory > 0) {
          setError(`Database Exhausted: All ${totalInCategory} qualifying names for ${category} have been shown.`);
        } else {
          setError(`No ${category} found in local vault matching criteria.`);
        }
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        // Provide a list of "avoid" words to the AI if possible, or just keep it simple
        const avoidList = Array.from(usedWords).slice(-10).join(', ');
        const prompt = `Provide one random Bollywood ${category} name that has at least ${minWords} words. 
        IMPORTANT: Do not provide any of these recently shown names: [${avoidList}]. 
        ...
        // Safety check if AI repeats itself
        if (usedWords.has(name.toLowerCase())) {
           // Small recursive retry once if it fails
           handleGenerate();
           return;
        }
        ...
        setError(err.message || "Failed to connect to the Internet Engine.");
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="min-h-screen flex flex-col items-center p-6 md:p-12 relative bg-[#050811] text-slate-100 selection:bg-amber-500/40 selection:text-white">
      ...
      <header className="text-center mb-12 z-10 animate-fade">
        ...
        <div className="flex items-center justify-center gap-4 mt-2">
          <p className="text-[10px] uppercase tracking-[0.3em] text-slate-500 font-semibold">Protocol V2.1</p>
        ...
        <div className="glass-panel p-8 md:p-12 rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] space-y-10 border border-white/5 relative overflow-hidden">
          ...
          <div className="flex flex-col space-y-3">
            <div className="flex justify-between items-center px-2">
               ...
               <button onClick={resetEngine} className="text-[9px] uppercase tracking-widest text-rose-500/50 hover:text-rose-400 transition-colors font-bold">Reset Session</button>
            </div>
          ...
          <div className="text-center">
            ...
            >
              ...
              {showVault ? 'Lock Vault' : 'Open Local Vault'}
            </button>
          </div>
