...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (searchMode === 'local') {
      ...
      try {
        // Safe check for process.env to prevent crashes on GitHub Pages
        const apiKey = typeof process !== 'undefined' ? process.env.API_KEY : null;
        
        if (!apiKey) {
          throw new Error("API Key not found. Internet mode requires 'API_KEY' environment variable. (Note: GitHub Pages does not support environment variables; use Vercel for Internet Mode).");
        }
            ...
            <div className={`p-8 md:p-12 rounded-[2rem] border-2 text-center animate-fade ${error ? 'border-red-500/20 bg-red-500/5 text-red-400' : 'border-amber-500/20 bg-amber-500/5'}`}>
              {error ? (
                <div className="space-y-3">
                  ...
                  <p className="text-[11px] uppercase tracking-widest leading-relaxed font-semibold max-w-xs mx-auto">{error}</p>
                </div>
              ) : (
